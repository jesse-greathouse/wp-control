...
package WPControl::Install::Rocky;
...
my @systemDependencies = qw(
    ...
    gcc gcc-c++ libstdc++-devel curl pkgconf-pkg-config perl-App-cpanminus
    ncurses-devel pcre-devel libcurl-devel ImageMagick-devel
    openssl-devel libxslt-devel mariadb-devel libxml2 libxml2-devel
    libicu-devel ImageMagick-c++-devel libzip-devel oniguruma-devel
    libsodium-devel glib2-devel libwebp-devel mariadb imagemagick zip
);
...
sub install_system_dependencies {
    ...
    my @installCmd = (
        'sudo', 'dnf', 'install', '-y', @systemDependencies
    );
    system(@installCmd);
    command_result($?, $!, "Installed Rocky Linux system dependencies...", \@installCmd);
}
