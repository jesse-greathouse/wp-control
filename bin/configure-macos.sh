...
if  [ "${CORRECT}" == "y" ]; then
    ...
    sed -i '' "s __DIR__ $DIR g" ${PHP_FPM_CONF}
    sed -i '' s/__SITE_NAME__/"${SITE_NAME}"/g ${PHP_FPM_CONF}
    sed -i '' s/__USER__/"${USER}"/g ${PHP_FPM_CONF}
    ...
    sed -i '' '1 a\
        PATH=/usr/local/bin:$PATH' ${WP_CLI_SCRIPT}
    ...
    sed -i '' "s __WEB__ $WEB g" ${NGINX_CONF}
    sed -i '' "s __VAR__ $VAR g" ${NGINX_CONF}
